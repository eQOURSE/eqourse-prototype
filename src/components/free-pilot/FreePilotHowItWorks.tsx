      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="hidden md:block relative">
            <div className="absolute top-[45px] left-[16%] right-[16%] h-[3px] bg-border/60 z-0 rounded-full overflow-hidden">
              <div
                className={`absolute top-0 left-0 h-full bg-gradient-to-r from-primary to-[#1B9AAA] rounded-full transition-all ease-out z-10 ${isVisible ? "w-full duration-[2s]" : "w-0"
                  }`}
              />
          ...
          <div className="block md:hidden absolute left-[58px] top-[240px] bottom-[240px] h-[60%] w-[3px] bg-border/60 z-0 rounded-full overflow-hidden">
            <div
              className={`absolute top-0 left-0 w-full bg-gradient-to-b from-primary to-[#1B9AAA] rounded-full transition-all ease-out z-10 ${isVisible ? "h-full duration-[2s]" : "h-0"
                }`}
            />
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-12 md:gap-8 relative z-10">
            {steps.map((step, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`flex-1 flex flex-row md:flex-col items-start md:items-center text-left md:text-center transition-all duration-700 ease-out ${isActive ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                    }`}
                >
                  ...
                  <div className="relative mr-6 md:mr-0 md:mb-8 shrink-0">
                    ...
                    <div
                      className={`w-[88px] h-[88px] rounded-full flex items-center justify-center transition-all duration-700 relative ${isActive
                    ...
                    {/* Step Number Badge */}
                    <div
                      className={`absolute -top-1 -right-1 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white shadow-md transition-all duration-500 ${isActive ? "bg-primary scale-100" : "bg-muted scale-0"
                        }`}
                    >
                      {step.number}
                    </div>
