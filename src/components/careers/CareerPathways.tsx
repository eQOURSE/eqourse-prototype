...
export default function CareerPathways() {
  const [active, setActive] = useState<"talent" | "vendor">("talent");
  return (
        ...
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-16">
          <div>
            ...
            <div className="mt-9 space-y-2" role="tablist" aria-label="Application type">
              <button role="tab" aria-selected={active === "talent"} onClick={() => setActive("talent")} className={`flex w-full items-start gap-4 border-l-2 px-5 py-5 text-left transition ${active === "talent" ? "border-teal-600 bg-teal-50 text-slate-950" : "border-slate-200 text-slate-500 hover:border-slate-400"}`}>
              ...
              <button role="tab" aria-selected={active === "vendor"} onClick={() => setActive("vendor")} className={`flex w-full items-start gap-4 border-l-2 px-5 py-5 text-left transition ${active === "vendor" ? "border-teal-600 bg-teal-50 text-slate-950" : "border-slate-200 text-slate-500 hover:border-slate-400"}`}>
          ...
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm sm:p-8 lg:p-10">
            <div className="mb-7">
